...
TEST(ContainerCanRegisterType, CaseTransient)
{
	...
	// When
	container.registerType<DependencyExample1>(Lifetime::TRANSIENT);
...
TEST(ContainerCanRegisterType, CaseSingleton)
{
	...
	// When
	container.registerType<DependencyExample1>(Lifetime::SINGLETON);
...
TEST(ContainerCanConstructTransient, Case1)
{
	...
	DependencyInjectionContainer container;
	container.registerType<DependencyExample1>(Lifetime::TRANSIENT);
	container.registerType<DependencyExample2>(Lifetime::TRANSIENT);
	container.registerType<DependentExample>(Lifetime::TRANSIENT);
...
TEST(ContainerCanConstructSingleton, Case1)
{
	...
	container.registerType<DependencyExample1>(Lifetime::TRANSIENT);
	container.registerType<DependencyExample2>(Lifetime::TRANSIENT);
	container.registerType<DependentExample>(Lifetime::SINGLETON);
